...
"""
import argparse, tempfile, itertools, setuptools, yaml
import logging
import os
import shutil
...
def go(args):
    ...
    #auc = roc_auc_score(y_val, pred_proba, average="macro", multi_class='ovo')
    ...
    #    export_model(run, pipe, processed_features, X_train, X_val, y_pred, args.output_artifact)

    signature=infer_signature(X_val[processed_features], y_pred)
    with tempfile.TemporaryDirectory() as temp_dir: 
        export_path = os.path.join(temp_dir, "random_forest_dir") # make file in temp path
        mlflow.sklearn.save_model(sk_pipe, path=export_path, signature=signature,
            input_example=X_val.iloc[:2], #sample of one row
...
def get_inference_pipeline(rf_config, max_tfidf_features):
    ...
    zero_imputer = SimpleImputer(strategy="most_frequent", fill_value=0)
    ...
    sk_pipe = Pipeline(
        steps=[
            ...
            ("random_forest", random_Forest),#**model_config["random_forest"])),
        ]
    )
